...
export default function JournalDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await api(`/journals/${id}`);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <Link to="/journals" className="text-sm text-slate-300 hover:text-white underline underline-offset-4">
        ← Back to journals
      </Link>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {item.volume ? <Pill>{item.volume}</Pill> : null}
              {item.issue ? <Pill>{item.issue}</Pill> : null}
            </div>
          </div>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-3xl border border-white/10 bg-slate-950/30 overflow-hidden">
            {pdf ? (
              ...
              <div className="p-6 text-slate-300">No PDF uploaded for this journal yet.</div>
            )}
          </div>
        </div>
      </div>
